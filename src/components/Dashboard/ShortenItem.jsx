...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate }) => {
    ...
    const analyticsHandler = (shortUrl) => {
        if (!analyticToggle) {
            ...
            // Reset analytics data when closing
            setAnalyticsData([]);
        }
    ...
    const fetchMyShortUrl = async () => {
        ...
        try {
            ...
            // Transform the data to match the expected format for the Graph component
            ...
            setSelectedUrl("");
            console.log("Analytics data:", transformedData);
            
        } catch (error) {
            console.error("Analytics fetch error:", error);
            // Don't navigate to error page for analytics failures, just show empty data
            setAnalyticsData([]);
            setSelectedUrl("");
        } finally {
            setLoader(false);
    ...
    <div className={`flex sm:flex-row flex-col  sm:justify-between w-full sm:gap-0 gap-5 py-5 `}>
      <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden ">
        <div className="text-slate-900 pb-1 sm:pb-0   flex items-center gap-2 ">
            ...
            <a
              href={import.meta.env.VITE_BACKEND_URL + "/api/s/" + `${shortUrl}`}
              target='_blank'
        ...
        <div className="flex  flex-1  sm:justify-end items-center gap-4">
            <CopyToClipboard
                ...
                text={`${import.meta.env.VITE_BACKEND_URL + "/api/s/" + `${shortUrl}`}`}
            >
